...
internal object MatrixUtil {
    ...
    private val POSITION_ADJUSTMENT_PATTERN_COORDINATE_TABLE =
        arrayOf<IntArray>(
            intArrayOf(-1, -1, -1, -1, -1, -1, -1), // Version 1
    ...
    fun embedTypeInfo(
        ...
        val typeInfoBits: BitArray = BitArray()
        makeTypeInfoBits(ecLevel, maskPattern, typeInfoBits)

        for (i in 0..<typeInfoBits.size) {
            // Place bits in LSB to MSB order.  LSB (least significant bit) is the last value in
            // "typeInfoBits".
            val bit: Boolean = typeInfoBits.get(typeInfoBits.size - 1 - i)
    ...
    fun maybeEmbedVersionInfo(
        ...
        val versionInfoBits: BitArray = BitArray()
        ...
        for (i in 0..5) {
            for (j in 0..2) {
                ...
                val bit: Boolean = versionInfoBits.get(bitIndex)
                bitIndex--
                // Left bottom corner.
    ...
    // - findMSBSet(1) => 1
    // - findMSBSet(255) => 8
    fun findMSBSet(value: Int): Int = 32 - Integer.numberOfLeadingZeros(value)
    ...
    fun makeTypeInfoBits(
        ...
        val typeInfo: Int = (ecLevel.bits shl 3) or maskPattern
        bits.appendBits(typeInfo, 5)

        val bchCode = calculateBCHCode(typeInfo, TYPE_INFO_POLY)
        bits.appendBits(bchCode, 10)

        val maskBits: BitArray = BitArray()
    ...
    private fun isEmpty(value: Byte): Boolean = value == (-1).toByte()
    ...
    private fun embedDarkDotAtLeftBottomCorner(matrix: ByteMatrix) {
        if (matrix.get(8, matrix.height - 8) == 0.toByte()) {
    ...
    private fun maybeEmbedPositionAdjustmentPatterns(
        ...
        val index: Int = version.versionNumber - 1
