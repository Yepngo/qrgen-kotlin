 ...
 * @author Sean Owen
 */
internal class BitArray : Cloneable {
    /**
    ...
    private fun ensureCapacity(newSize: Int) {
        if (newSize > bitArray.size * 32) {
            val newBits: IntArray = makeArray(ceil((newSize / LOAD_FACTOR).toDouble()).toInt())
            System.arraycopy(this.bitArray, 0, newBits, 0, bitArray.size)
     ...
     */
    fun get(i: Int): Boolean = (this.bitArray[i / 32] and (1 shl (i and 0x1F))) != 0
     ...
     */
    fun set(i: Int) {
        this.bitArray[i / 32] = this.bitArray[i / 32] or (1 shl (i and 0x1F))
    }
     ...
     */
    fun flip(i: Int) {
        this.bitArray[i / 32] = this.bitArray[i / 32] xor (1 shl (i and 0x1F))
    }
    ...
    fun getNextSet(from: Int): Int {
        ...
        var currentBits = this.bitArray[bitsOffset]
        ...
        while (currentBits == 0) {
            ...
            currentBits = this.bitArray[bitsOffset]
        }
    ...
    fun getNextUnset(from: Int): Int {
        ...
        var bitsOffset = from / 32
        var currentBits = this.bitArray[bitsOffset].inv()
        // mask off lesser bits first
        currentBits = currentBits and -(1 shl (from and 0x1F))
        while (currentBits == 0) {
            if (++bitsOffset == bitArray.size) {
            ...
            currentBits = this.bitArray[bitsOffset].inv()
        }
        val result = (bitsOffset * 32) + Integer.numberOfTrailingZeros(currentBits)
        return min(result, size)
    }
    ...
    fun setRange(
        ...
        for (i in firstInt..lastInt) {
            ...
            this.bitArray[i] = this.bitArray[i] or mask
        }
    }
    ...
    fun isRange(
        ...
        for (i in firstInt..lastInt) {
            ...
            if ((this.bitArray[i] and mask) != (if (value) mask else 0)) {
                return false
            }
        }
        return true
    ...
    fun appendBit(bit: Boolean) {
        ...
        if (bit) {
            this.bitArray[size / 32] = this.bitArray[size / 32] or (1 shl (size and 0x1F))
        }
        size++
    }
    ...
    fun appendBits(
        ...
        for (numBitsLeft in numBits - 1 downTo 0) {
            if ((value and (1 shl numBitsLeft)) != 0) {
                this.bitArray[nextSize / 32] = this.bitArray[nextSize / 32] or (1 shl (nextSize and 0x1F))
            }
            nextSize++
        }
        size = nextSize
    ...
    fun xor(other: BitArray) {
        ...
        for (i in bitArray.indices) {
            ...
            this.bitArray[i] = this.bitArray[i] xor other.bitArray[i]
        }
    }
    ...
    fun reverse() {
        ...
        for (i in 0..<oldBitsLen) {
            newBits[len - i] = Integer.reverse(this.bitArray[i])
        }
    ...
    companion object {
        private val EMPTY_BITS = intArrayOf()
