 ...
 * @author Sean Owen
 */
internal enum class CharacterSetECI {
    ...
    val charset: Charset
        get() = Charset.forName(name)

    companion object {
        // only character sets supported by the current JVM are registered here
        private val NAME_TO_ECI: MutableMap<String?, CharacterSetECI?> = HashMap<String?, CharacterSetECI?>()
         ...
         * but unsupported
         */
        fun getCharacterSetECI(charset: Charset): CharacterSetECI? = NAME_TO_ECI.get(charset.name())
         ...
         * but unsupported
         */
        fun getCharacterSetECIByName(name: String?): CharacterSetECI? = NAME_TO_ECI.get(name)
    }
}
