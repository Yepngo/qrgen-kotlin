...
import com.yepngo.qrgen.internal.zxing.common.CharacterSetECI
import com.yepngo.qrgen.internal.zxing.common.StringUtils
import com.yepngo.qrgen.internal.zxing.common.reedsolomon.GenericGF
import com.yepngo.qrgen.internal.zxing.common.reedsolomon.ReedSolomonEncoder
...
import com.yepngo.qrgen.internal.zxing.qrcode.encoder.MaskUtil.applyMaskPenaltyRule1
import com.yepngo.qrgen.internal.zxing.qrcode.encoder.MaskUtil.applyMaskPenaltyRule2
import com.yepngo.qrgen.internal.zxing.qrcode.encoder.MaskUtil.applyMaskPenaltyRule3
import com.yepngo.qrgen.internal.zxing.qrcode.encoder.MaskUtil.applyMaskPenaltyRule4
import com.yepngo.qrgen.internal.zxing.qrcode.encoder.MinimalEncoder.Companion.encode
...
internal object Encoder {
    ...
    val DEFAULT_BYTE_MODE_ENCODING: Charset = StandardCharsets.ISO_8859_1
    ...
    fun encode(
        ...
        val hasGS1FormatHint =
            hints != null && hints.containsKey(EncodeHintType.GS1_FORMAT) &&
                hints
                    .get(EncodeHintType.GS1_FORMAT)
                    .toString()
                    .toBoolean()
        val hasCompactionHint =
            hints != null && hints.containsKey(EncodeHintType.QR_COMPACT) &&
                hints
                    .get(EncodeHintType.QR_COMPACT)
                    .toString()
                    .toBoolean()
        ...
        if (hasCompactionHint) {
            ...
            val priorityEncoding = if (encoding == DEFAULT_BYTE_MODE_ENCODING) null else encoding
            val rn = encode(content, null, priorityEncoding, hasGS1FormatHint, ecLevel)
            ...
            if (mode == Mode.BYTE && hasEncodingHint) {
                val eci = CharacterSetECI.getCharacterSetECI(encoding)
                if (eci != null) {
            ...
            if (hints != null && hints.containsKey(EncodeHintType.QR_VERSION)) {
                val versionNumber = hints.get(EncodeHintType.QR_VERSION).toString().toInt()
                version = getVersionForNumber(versionNumber)
                val bitsNeeded = Encoder.calculateBitsNeeded(mode, headerBits, dataBits, version!!)
        ...
        if (hints != null && hints.containsKey(EncodeHintType.QR_MASK_PATTERN)) {
            val hintMaskPattern = hints.get(EncodeHintType.QR_MASK_PATTERN).toString().toInt()
    ...
    private fun recommendVersion(
        ...
        // the minimum, 1:
        val provisionalBitsNeeded = Encoder.calculateBitsNeeded(mode, headerBits, dataBits, getVersionForNumber(1)!!)
    ...
    fun chooseMode(content: String): Mode = chooseMode(content, null)
    ...
    private fun chooseMode(
        ...
        if (StringUtils.SHIFT_JIS_CHARSET != null &&
            StringUtils.SHIFT_JIS_CHARSET == encoding &&
    ...
    fun isOnlyDoubleByteKanji(content: String): Boolean {
        val bytes = content.toByteArray(StringUtils.SHIFT_JIS_CHARSET!!)
    ...
    fun interleaveWithECBytes(
        ...
        val blocks: MutableCollection<BlockPair> = ArrayList<BlockPair>(numRSBlocks)
    ...
    fun appendNumericBytes(
        ...
        var i = 0
        while (i < length) {
            val num1 = content.get(i).code - '0'.code
            if (i + 2 < length) {
                // Encode three numeric letters in ten bits.
                val num2 = content.get(i + 1).code - '0'.code
                val num3 = content.get(i + 2).code - '0'.code
    ...
    fun appendAlphanumericBytes(
        ...
        var i = 0
        while (i < length) {
            val code1 = getAlphanumericCode(content.get(i).code)
            if (code1 == -1) {
                throw WriterException()
            }
            if (i + 1 < length) {
                val code2 = getAlphanumericCode(content.get(i + 1).code)
                if (code2 == -1) {
                    throw WriterException()
                }
    ...
    fun appendKanjiBytes(
        ...
        if (StringUtils.SHIFT_JIS_CHARSET == null) {
            ...
            throw WriterException("SJIS Charset not supported on this platform")
        }
        val bytes = content.toByteArray(StringUtils.SHIFT_JIS_CHARSET)
        if (bytes.size % 2 != 0) {
